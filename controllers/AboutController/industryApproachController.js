...
exports.getAllPages = async (req, res) => {
    try {
        const pages = await IndustryApproachPage.find().select('title slug createdAt');
...
exports.createPage = async (req, res) => {
    try {
        ...
        const exists = await IndustryApproachPage.findOne({ slug });
        if (exists) return res.status(400).json({ message: "Page with this title already exists" });

        const newPage = new IndustryApproachPage({ title, slug, ...req.body });
        await newPage.save();
...
exports.getPageBySlug = async (req, res) => {
    try {
        ...
        const page = await IndustryApproachPage.findOne({ slug });
...
exports.deletePage = async (req, res) => {
    try {
        const { id } = req.params;
        await IndustryApproachPage.findByIdAndDelete(id);
